...
import { buildReminderSlots, getUpcomingSlotsForToday } from '../src/utils/reminderSlots.ts';
...
test('getUpcomingSlotsForToday retorna apenas slots futuros', () => {
  const slots = [480, 540, 600, 660];
  assert.deepEqual(getUpcomingSlotsForToday(slots, 600), [660]);
});
